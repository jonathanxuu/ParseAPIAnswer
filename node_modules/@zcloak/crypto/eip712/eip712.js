 ...
 */
export function getDependencies(typedData, type, dependencies = []) {
 ...
 */
export function encodeType(typedData, type) {
 ...
 */
export function typeHash(typedData, type) {
...
function encodeValue(typedData, type, data) {
  ...
  if (type === 'string') {
    return ['bytes32', keccak256AsU8a(data)];
  }
  if (type === 'bytes') {
    return ['bytes32', keccak256AsU8a(u8aToU8a(data))];
  }
  return [type, data];
}
 ...
 */
export function encodeData(typedData, type, data) {
 ...
 */
export function structHash(typedData, type, data) {
 ...
 * with Keccak256.
 */
export function getMessage(typedData, hash) {
  const message = u8aConcat(EIP_191_PREFIX, structHash(typedData, 'EIP712Domain', typedData.domain), structHash(typedData, typedData.primaryType, typedData.message));
  if (hash) {
